...
def clean_vax():
  #cleans the vaccine dataset, pulling out the most recent data values for people vaxinated per 100 people for each state 
  ...
  for dict in data:
    if(dict["date"] == "2022-11-02" and dict["location"] in us_states):
      people_vax_ph.append(dict["people_vaccinated_per_hundred"])
...
def clean_test():
  #cleans the standadrized testing dataset by pulling out all the average scores for each state
  #returns it as a series
...
def chart():
  ...
  frame = { 'Average Scores on Standardized English Test': test_series, 'People Vaccinated Per Hundred': vaccine_series }
